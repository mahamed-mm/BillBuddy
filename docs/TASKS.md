...
- [x] **BF-1** Currency picker flags and rounding-row clipping on iOS 26 (reported 2026-09-25 from a simulator screenshot) · deps: — · 9cd3765 + ab13cb8, code review APPROVED 5/5 and design review Mode B APPROVED in round 2 (round 1 [B1]: dead tap bands on the widened pills; found by both reviewers and CodeRabbit)
  ...
  - Backlog (from the BF-1 reviews):
    ...
    - In light previews, selected chip text (`bbTeal` on `bbSelectedChip`) is 1.4:1 (design N4). Forced dark hides it at runtime. 2A-18 added the `bbTealText` token; switching the chips to it is [2A-18 design N12] in the Phase 2A backlog.
...
- **Q10** (2026-09-26, after 2A-18's code review [B1]) AccentColor stays a single `#00E5CC` in every appearance. On iOS 17 the keyboard toolbar resolves AccentColor from the device's appearance, outside the forced-dark window, so the light variant from spec §5's developer note turned "Done" `#00695C` on the dark toolbar (2.8:1) on light-mode devices. Light previews keep the bright teal caret. Forcing dark app-wide in Info.plist was considered and not chosen; it would also turn the launch screen black for light-mode users.
- **Q11** (2026-09-26, 2A-5) Tip ↑ and Total ↑ round up the amount as shown, after half-up rounding to the øre/cent: a bill of 6,67 at 15 % keeps a tip of 1,00 under Tip ↑, and 0,87 at 15 % keeps a total of 1,00 under Total ↑. (The old `Double` math rounded the hidden fraction up and showed 2,00.)
...
- [x] **2A-5** ViewModel: tip and total in exact minor units, with an input cap (~2h) · deps: 2A-1, 2A-2, 2A-4 · ee2ba7c (reviewed as 2ccf2a3), code review APPROVED 5/5, 0 fix rounds; 119/119 on iOS 17.5 and 26.3.1
  ...
  - AC: tip and total are computed as Int minor units (tip rounded half-up to 1 øre/cent; Tip ↑ and Total ↑ round up to a whole unit as today, starting from the amount as shown, Q11), and `tipAmount` and `totalAmount` derive from them
...
- [x] **2A-6** ViewModel: custom split state and automatic rows (~2h) · deps: 2A-0, 2A-3, 2A-5 · 2ceeef5 (reviewed as ab3dd49), code review APPROVED 5/5, 0 fix rounds; `splitCount` is now computed from `personSplits` (clamped to `splitCountRange` 1…20), and the outputs are `billPortionsMinorUnits` and `automaticAmountText(for:)`
  ...
  - AC: examples: bill "3000", 2 people → [150_000, 150_000]; bill "1250", 4 people, Person 1 "150" → [15_000, 36_667, 36_667, 36_666]; 4 automatic rows at 31_250, then + → 5 rows at 25_000; typed rows over the bill → automatic rows 0; no bill → every automatic row 0, while a typed row keeps its amount (spec §3.1; S1a comes first in §3.2, so nothing visible differs; 2A-7 and 2A-16 read it the same way, [2A-6 N4]); a row of `"  "` is automatic
  ...
  - AC ([2A-5 N2, N5]) clean-up before the row tasks: `AmountParser.amount(from:)` (uncapped, with no app caller since 2A-5) is removed, and its tests move onto `minorUnits(from:currency:)`, so 2A-10 and 2A-16 can't decide validity without the cap; the `tipBasisPoints` doc and its test name say "to the nearest basis point" (a tie past 2 decimals can go either way, because the product is binary)
...
- [ ] **2A-9** `SplitModeToggle` in `SplitControlView` (~2h) · deps: 2A-0, 2A-6, 2A-15, 2A-18
  ...
  - AC ([2A-6 N3]): `SplitControlView`'s hard-coded bounds 1 and 20 (disabled states and colors) come from `CalculatorViewModel.splitCountRange`, so the stepper, E10, and the "Adds Person N+1" hint share one bound
...
- [ ] **2A-11** Custom split list and pinned status card (~3h) · deps: 2A-6, 2A-9, 2A-10, 2A-15, 2A-16, 2A-17, 2A-19
  - AC: in Custom mode only, `SplitControlView` shows the §4.3 custom block in order: rule caption (§4.9), one `PersonSplitRow` per person via `ForEach($viewModel.personSplits)` (stable ids with element bindings, never index bindings), then the footer; each row's `automaticAmountText` comes from 2A-6's formatter, read in one pass per body rather than one `automaticAmountText(for:)` call per row. For example, add a `[PersonSplit.ID: String]` built from a single `billPortionsMinorUnits` read: 0.04 ms instead of 0.5 ms per 20-row redraw, on the keystroke path ([2A-6 N1])
...
- [ ] **2A-12** Keyboard focus flow across bill and person fields (~2h) · deps: 2A-11, 2A-15, 2A-19
  ...
  - AC (Q10, 2A-18 [B1]; amended 2026-09-26, the product owner now tests on iOS 26.3+ only): `AccentColorTests` (a single `#00E5CC` under light, dark, and unspecified traits) passes. It is the guard for the iOS 17 light-mode toolbar, which can no longer be checked on a simulator. On iOS 26.3.1 the toolbar items are the system's neutral Liquid Glass buttons, which don't take AccentColor, and stay as the system draws them. The layout follows 2A-19's amended §4.1; the PR includes iOS 26.3.1 screenshots
...
- [ ] **2A-14** Docs and manual QA sweep (~1h) · deps: 2A-1 to 2A-13, 2A-15 to 2A-18
  ...
  - AC: manual QA with Region = Norway on an iOS 26.3 simulator (the product owner tests on iOS 26.3+ only, since 2026-09-26): bill "12,50" computes, and a 3-person custom split works end to end in all 4 rounding modes, starting from the automatic equal split. The deployment target stays iOS 17.0 (Q9), so the PR notes that iOS 17 isn't run; `AccentColorTests` guards Q10
...
- [ ] **2A-17** `SplitStatusView` (~1.5h) · deps: 2A-15, 2A-16, 2A-18, 2A-19
...
- [x] **2A-18** Land the five 2A design tokens (~1h) · deps: — · ‖ · f19c2f5 + 0e4c927 (reviewed as 4f62b41 + bab0381); code review APPROVED 5/5 after 1 fix round ([B1]: the AccentColor light variant, Q10); design review Mode B APPROVED in round 1 and on re-review
  ...
  - AC (amended by Q10 after code review [B1]): AccentColor stays a single `#00E5CC` in every appearance (no light variant), a test pins it under light, dark, and unspecified traits, and nothing changes at runtime. The original AC asked for a light appearance of `#00695C` (spec §5 developer note), which turned the iOS 17 keyboard toolbar's Done dark teal on light-mode devices
...
- [ ] **2A-19** Spec amendment: iOS 26's floating keyboard toolbar (~1h) · deps: — · blocks 2A-11, 2A-12, 2A-17 · added by the PM on 2026-09-26, after 2A-18's design re-review and the move to iOS 26.3-only testing
  - AC (ui-designer → design-reviewer Mode A, APPROVED within 3 rounds): spec §4.1 places the pinned status card and the keyboard controls correctly on iOS 26.3.1. There, "Done" is a floating Liquid Glass capsule that overlaps scroll content, while §4.1:225 and §6:652 assume iOS 17's full-width bar with the card "directly above" it. Check it against iOS 26.3.1 screenshots, with the bill field and a person field focused, at default text size and AX5 ([2A-18 design N14])
  - AC: spec §4.1:250 and STYLE-GUIDE:45 say that on iOS 26 the system draws keyboard-toolbar items as neutral Liquid Glass buttons that don't take AccentColor, and that they're left as drawn ([2A-18 design N8]); the ∧/∨ chevrons are covered too
  - AC: folds in the 2A-18 wording nits ([2A-18 design N9–N11], code re-review N4):
    - STYLE-GUIDE:45 says "the `#00695C` light variant showed on the dark toolbar at about 2.8:1"
    - spec §5:609 says "…the bright teal caret and dialog buttons"
    - the STYLE-GUIDE:44 citation reads "§4.5 and §4.7"
    - STYLE-GUIDE:46 says to "add the `UIColor` to `AppColors` with its own dynamic provider"
    - spec §8's review history gets a line for the Q10 amendment
...
- [2A-1 N3] Resolved by 2A-5 (ee2ba7c): the `minorUnits(from:fractionDigits:)` seam is tested at 0 and 3 digits (`",5"`@0 → 1, `"1,0005"`@3 → 1001), plus a negative scale and the cap at both scales.
- [2A-1 N4] TESTING.md still says 39 tests (155 now; its line 9, just above 2A-2's new "Test Isolation" section, per [2A-2 N2]) and has no AmountParser section. Its manual QA lacks a "type 12,50 with Region = Norway" step, which should be checked before tagging 2.0.1 (the rest is covered by 2A-14). CLAUDE.md also says 39 (human-owned).
...
- [2A-4 N4] Resolved by 2A-5 (ee2ba7c): ARCHITECTURE.md has the "Exact money in Int minor units" decision row.
...
- [2A-5 N3] A corrupted `savedCustomTip` (NaN, ±inf, or outside `Int`'s range) traps `Int(viewModel.customTipPercent)` in `TipSelectorView`, and a finite out-of-range value such as 60 shows "60%" while the tip is computed at 50 % (`calculation.tipPercent` also reports 60). Only a hand-edited preferences file reaches it. Clean the value once on restore: `customTipPercent = savedCustomTip.isFinite ? min(max(savedCustomTip, 0), 50) : 18`, and keep the basis-point clamp as the overflow guard.
- [2A-5 N4] The 50 % custom-tip limit lives in two places (the slider's `0...50` and `maxTipBasisPoints`), and it's also the overflow bound: at the cap, `billMinorUnits * tipBasisPoints + 5_000` overflows above 9 223 bp (92.23 %). Use one constant (for example `customTipRange = 0...50`) for the slider and the clamp, and note the 9 223 bp ceiling in its doc.
- [2A-5 N6] CHANGELOG (2A-14): the 2A-5 entry is accurate but is one long bullet that mixes a fix with two behavior changes. Keep the 33,30 → 5,00 / 38,30 fix and the more-than-two-decimals rounding under Fixed, and move "Tip ↑ and Total ↑ round up the amount as shown" (Q11) and the input cap to Changed, one short sentence each.
- [2A-5 N7] For 2A-8: equal-mode `perPersonAmount` still divides a `Double`, and NumberFormatter shows an exact half-øre per-person amount half-even (7,69 ÷ 2 → 3,84; 0,05 ÷ 2 → 0,02), unlike the half-up tip. Precision near the cap is fine. For one rounding rule, 2A-8 can move it to Int math: display `(2 * totalMinorUnits + n) / (2 * n)`, and Per Person ↑ `ShareAllocator.roundedUpToWholeUnit((totalMinorUnits + n - 1) / n, currency:)`. Ask the product owner before changing what equal mode shows.
- [2A-18 design N12] Switch the selected text of `TipPresetButton` and the rounding pills from `bbTeal` to `bbTealText` (1.4:1 → 5.6:1 in light previews; the same in dark). Update STYLE-GUIDE's TipPresetButton table (line 168) in the same change, and add the rule "teal text and glyphs use `bbTealText`, never `bbTeal`". Natural to bundle with 2A-9, whose `SplitModeToggle` chips already use `bbTealText`.
- [2A-6 N2] `CustomSplitTests.modeAndAmountsAreNotSaved` diffs the store's whole search list, which includes global keys that system daemons write. It had 0 mismatches in 300 repeats, but a daemon write in that window would fail it. Make `TestDefaults.suiteNames` `private(set)` and assert on the suite's own domain: `store.persistentDomain(forName:)` keys == the 5 preference keys.
- [2A-6 follow-ups] For 2A-16: the lowest invalid row, the typed sum, and "any automatic row" can share the typed-row step inside `billPortionsMinorUnits` instead of parsing the rows again. Assigning `personSplits` directly is the one path that could break the 1…N numbering; nothing does today, and `clearTypedAmounts()` must edit in place ([2A-3 N4b]). The `splitCount > 0` guards in `perPersonAmount` can no longer be reached (2A-8 can drop them).
- [2A-18 follow-ups] ARCHITECTURE.md's folder tree describes AppSpacing as "Spacing scale (xs–xxl) + corner radii" without `minTapTarget` (2A-14). The STYLE-GUIDE color table's "Token" header cell is one character narrower than its rows (cosmetic).
- [2A-5 follow-up] For 2A-16 and 2A-17: S3's excess can reach about 1.9 × 10^16 minor units (20 rows at the cap), past 2^53, so its displayed amount could be off by a few øre there. Harmless, but don't assert exact `Double` values at that size.
